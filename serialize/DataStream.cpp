...
void DataStream::write(const  unsigned char * data, int len) {
	reserve(len);
    int size = m_buf.size();
    m_buf.resize(m_buf.size() + len);
    std::memcpy(&m_buf[size], data, len);
...
void DataStream::write(const CImPdu & pdu) {	
	char type = DataType::BYTEARRY;
    write((char *)&type, sizeof(char));
	
	int len = pdu.GetLength();
	unsigned char* data = pdu.GetBuffer();
	write(len);
	write(data, len);	
}
...
bool DataStream::read(unsigned char * data, int len) {
	std::memcpy(data, (unsigned char *)&m_buf[m_pos], len);
    m_pos += len;
    return true;
}
...
bool DataStream::read(CImPdu & pdu) {
	if (m_buf[m_pos] != DataType::BYTEARRY)
    {
        return false;
    }
	 ++m_pos;
	...
	pdu = *pdu.ReadPdu((unsigned char*)&m_buf[m_pos], len);
    m_pos += len;
