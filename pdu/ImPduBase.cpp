...
uchar_t* CImPdu::GetBuffer() const
{
    return m_buf.GetBuffer();
}

uint32_t CImPdu::GetLength() const
{
    return m_buf.GetWriteOffset();
}

uchar_t* CImPdu::GetBodyData() const
{
    return m_buf.GetBuffer() + sizeof(PduHeader_t);
}

uint32_t CImPdu::GetBodyLength() const
{
...
{
    //设置包体，则需要重置下空间
    m_buf.Read(NULL, m_buf.GetWriteOffset()); //清空m_buf
    m_buf.Write(NULL, sizeof(PduHeader_t));   //预留出header空间
    ...
    {
        error("pb msg miss required fields.");
    }
