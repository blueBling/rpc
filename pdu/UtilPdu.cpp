...
CSimpleBuffer::CSimpleBuffer(const CSimpleBuffer& buf) {
	uint32_t size = buf.GetAllocSize();
	if(size <= 0) {
		m_buffer = NULL;
		m_alloc_size = 0;
		m_write_offset = 0;
	...
	this->m_buffer = (uchar_t*)malloc(size);
	uchar_t* buffer = buf.GetBuffer();
	memcpy(this->m_buffer, buffer, size);
	this->m_alloc_size = size;
	this->m_write_offset = buf.GetWriteOffset();
}
CSimpleBuffer & CSimpleBuffer::operator=(const CSimpleBuffer& buf) {
	if(this == &buf)
		return *this;
